...
configOptsParser hide0 =
    (\workDir buildOpts dockerOpts nixOpts systemGHC installGHC arch os ghcVariant jobs includes libs skipGHCCheck skipMsys localBin modifyCodePage -> mempty
...
buildOptsMonoidParser hide0 = undefined
    -- transform <$> trace <*> profile <*> options
  where
    hide = hideMods hide0
    transform tracing profiling = enable
          where enable opts
                  | tracing || profiling =
    ...
    trace =
            ...
            <> help "Enable profiling in libraries, executables, etc. \
                    ...
                    \exception")
    options = undefined
        -- BuildOptsMonoid <$> target <*> libProfiling <*> exeProfiling <*>
